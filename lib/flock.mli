...
(** [flock ~nonblocking fd operation] applies or removes an advisory lock on
    ...
    means that duplicate file descriptors (created by, for example, [Unix.fork]
    or [Unix.dup]) refer to the same lock, and this lock may be modified or
    ...
    If a process uses [Unix.openfile] (or similar) to obtain more than one
    descriptor for the same file, these descriptors are treated independently
    by [flock]. An attempt to lock the file using one of these file descriptors
    ...
    Locks created by [flock] are preserved across a [Unix.execve].
